...
nodejs / python / java / go 四种容器环境的一键部署（四协议游戏机 + 哪吒探针），附 VPS / Serv00 一键脚本。
四协议组合：vless-reality | vmess-ws-tls(argo) | hysteria2 | tuic5

## 目录结构

| 目录 | 说明 | 部署入口 |
|---|---|---|
| `nodejs/` | Node.js 容器版 | `node index.js` |
| `python/` | Python 容器版 | `python app.py` |
| `java/` | Java 容器版（server.jar + .env 配置模板） | `java -jar server.jar` |
| `go/` | Go 容器版（HTTP 服务 + 调用 start.sh） | `go build` 后运行 |
| `scripts/` | VPS / Serv00 一键脚本（sing-box.sh / sb_00.sh / x00.sh / sb3.sh） | `bash scripts/xxx.sh` |

节点输出：运行后生成 `world/sub.txt`（Base64 订阅，导入 v2rayN / NekoBox / 小火箭）与 `world/list.txt`（明文链接）。
